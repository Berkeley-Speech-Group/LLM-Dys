...
def process_data(start_id, end_id, gpu_id, speaker_id, json_path, output_dir):
    ...
    for i in range(start_id, end_id + 1):
        ...
        item = data[i]
        text = item['ipa_dysfluent_re'] if 'ipa_dysfluent_re' in item else item['ipa_dysfluent']
        file_id = str(item['id'])
...
if __name__ == '__main__':
    type = "rep"  # [rep, del, sub, ins]"
    parser = argparse.ArgumentParser()
    parser.add_argument('--start_id', type=int, required=True)
    parser.add_argument('--end_id', type=int, required=True) 
    parser.add_argument('--gpu_id', type=int, required=True)
    parser.add_argument('--speaker_id', type=int, required=True)
    parser.add_argument('--json', type=str, default=f"/set your path/LLM-Dys/labels/phn_{type}.json")
    parser.add_argument('--output', type=str, default=f"/your path to save the dataset/LLM_dys/phn/phn_{type}")

...
# setproctitle.setproctitle("0.005_r0.05_phn")
...
def calculate_token_distance(prediction, label):
    def get_token_positions(text):
        return [(m.group(1), m.start()) for m in re.finditer(r'\[(REP|PAU|PRO)\]', text)]
...
def calculate_metrics_by_type(predictions, labels, token_type=None):
    ...
    for pred, label in zip(predictions, labels):
        ...
        if token_type:
            ...
            pred_tokens = [(m.group(1), m.start()) for m in re.finditer(r'\[(REP|PAU|PRO)\]', pred)]
            label_tokens = [(m.group(1), m.start()) for m in re.finditer(r'\[(REP|PAU|PRO)\]', label)]
...
def compute_metrics(pred):
    ...
    type_counts = {
        ...
        'PRO': sum(1 for label in label_str if '[PRO]' in label)
    }
    total_samples = len(label_str)
    total_dys = sum(type_counts.values())
    
    # Calculate metrics for each type
    for token_type in ['REP', 'PAU', 'PRO']:
    ...
    type_weights = {
        ...
        'PRO': type_counts['PRO'] / total_dys if total_dys > 0 else 0
    }
    ...
    metrics.update({
        ...
        'pro_precision': type_results['pro']['precision'],
        'pro_recall': type_results['pro']['recall'],
        'pro_f1': type_results['pro']['f1'],
        'pro_ter': type_results['pro']['ter'],
        'pro_td': type_results['pro']['td']
    })
...
def main():
    ...
    csv_files_with_ratios = {
        '/data/jinming/LLM_dys/csv/train/phn_pro_4000.csv': 1,
        '/data/jinming/LLM_dys/csv/train/phn_rep_4000.csv': 1,
        '/data/jinming/LLM_dys/csv/train/phn_pau_4000.csv': 0.3, 
        '/data/jinming/LLM_dys/csv/train/word_pau_4000_w2p.csv':0.7,   ## 240 0000*0.01=24000
        '/data/jinming/LLM_dys/csv/train/vctk_phn.csv': 0.015          ## 4 3800*0.03 = 1314
    }
...
python /home/jinming/token/train_phn/train_0.005_ratio0.05.py
